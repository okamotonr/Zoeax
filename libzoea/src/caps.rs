use crate::{syscall::{cnode_copy, cnode_mint, configure_tcb, map_page, map_page_table, recv_ipc, recv_signal, resume_tcb, send_ipc, send_signal, set_ipc_buffer, unmap_page, untyped_retype, write_reg, SysCallFailed}, IPCBuffer};
...
impl KernelObject for Untyped {
    ...
    fn from_retype(user_size: usize, is_device: bool) -> Self {
        Self { size_bits: user_size, is_device }
    }
}
...
impl KernelObject for Page {
    ...
    fn from_retype(_user_size: usize, _is_device: bool) -> Self {
        Self { mapped_address: 0, is_mapped: false, rights: PageFlags::never() }
    }
}
...
impl CNodeCapability {
    ...
    pub fn copy<K: KernelObject + Copyable>(&mut self, cap: &Capability<K>) -> Result<Capability<K>, SysCallFailed> {
        let (dest_ptr, dest_depth) = self.get_slot()?.get_cap_ptr();
        cnode_copy(self.cap_ptr, self.cap_depth, dest_ptr, dest_depth, cap.cap_ptr, cap.cap_depth)?;
    ...
    pub fn delete<K: KernelObject>(&mut self, mut _cap: Capability<K>) -> Result<(), SysCallFailed> {

        // kernel adaptation is not yet done.
        Ok(())
    }

    pub fn mint<K: KernelObject + Mintable>(&mut self, cap: &Capability<K>, cap_val: usize) -> Result<Capability<K>, SysCallFailed> {
        let (dest_ptr, dest_depth) = self.get_slot()?.get_cap_ptr();
        cnode_mint(self.cap_ptr, self.cap_depth, dest_ptr, dest_depth, cap.cap_ptr, cap.cap_depth, cap_val)?;
        let cap_data = K::mint_data(&cap.cap_data, cap_val);
        Ok(
            Capability { cap_ptr: dest_ptr, cap_depth: dest_depth, cap_data }
        )
    }
}
...
impl PageTableCapability {
    pub fn map(&mut self, root_table: &mut Self, vaddr: usize) -> Result<usize, SysCallFailed> {
        map_page_table(self.cap_ptr, self.cap_depth, root_table.cap_ptr, root_table.cap_depth, vaddr)
    }
}
...
impl PageCapability {
    pub fn map(&mut self, root_table: &mut PageTableCapability, vaddr: usize, flags: PageFlags) -> Result<(), SysCallFailed> {
        map_page(self.cap_ptr, self.cap_depth, root_table.cap_ptr, root_table.cap_depth, vaddr, flags.into())?;
    ...
    pub fn unmap(&mut self, root_table: &mut PageTableCapability) -> Result<(), SysCallFailed> {
        unmap_page(self.cap_ptr, self.cap_depth, root_table.cap_ptr, root_table.cap_depth)?;
...
impl From<PageFlags> for usize {
    fn from(value: PageFlags) -> Self {
        let write = if value.is_writable {
            0x02 
        } else { 0x00 };
        let read = if value.is_readable  {
            0x04
        } else { 0x00 };
        let exec = if value.is_executable {
            0x01
        } else { 0x00 };
        write | read | exec
    }
...
impl TCBCapability {
    ...
    pub fn set_ipc_buffer(&mut self, page_cap: &PageCapability
    ) -> Result<(), SysCallFailed> {
        set_ipc_buffer(self.cap_ptr, self.cap_depth, page_cap.cap_ptr, page_cap.cap_depth)?;
        Ok(())
    }

    pub fn write_regs<F: FnOnce() -> Registers>(&mut self, writer: F, ipc_buffer: &mut IPCBuffer) -> Result<(), SysCallFailed> {
        write_reg(self.cap_ptr, self.cap_depth, writer, ipc_buffer)?;
        Ok(())
    }

    pub fn configure(&mut self, root_cnode: &mut CNodeCapability, root_vspace: &mut PageTableCapability) -> Result<(), SysCallFailed> {
        configure_tcb(self.cap_ptr, self.cap_depth, root_cnode.cap_ptr, root_cnode.cap_depth, root_vspace.cap_ptr, root_vspace.cap_depth)?;
        Ok(())
    }
